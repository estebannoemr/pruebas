...
# ACLARACIÓN:
**Ʃ (SIGMA) es el ALFABETO de entrada (o UNIVERSAL) de una máquina de Turing, es decir, el conjunto de símbolos que forman las cadenas de entrada, por eso no incluye B.**

**Γ (GAMMA) representa el alfabeto de la CINTA de una máquina de Turing, es decir, el conjunto de símbolos que la máquina de Turing entiende y utiliza durante su cómputo, e incluye a B**
...
1. Desplazarse hacia la derecha en la cinta 1 y hacia la izquierda en la cinta 2. Reemplaza las "b" de la cinta 2 mientras haya una "c" en la cinta 1.
...
1. `δ(q0, (a, B)): qa, (a, R), (a, R)`: si valida que haya una "a" en la primera posición, comienza a escribir en la cinta 2. EXPLICACION: en la cinta 1 hay un símbolo "a" y en la cinta 2 hay un espacio en blanco. Cambio de estado a "qa"; escribo "a" en la cinta 1 (se deja igual) y se mueve a la derecha; escribo "a" en la cinta 1 (deja de ser B) y se mueve a la derecha.
...
Una MT sin el movimiento S puede simular a una que si lo tiene simplemente moviéndose a la derecha y luego a la izquierda de forma intermitente (o viceversa) sin alterar el estado o símbolo corriente.
...
Alfabeto Γ = { a, b, a', b', B }

Estados: Q = { q0, qa, qb, qr, qn, qA, qR }
* q0: estado inicial.
...
| qt | qa, a', R | qb, b', R | qA, a', S | qA, b', S | qA, B, S |
...
Estados: Q = { q0, q1, q2, qA, qR }

* q0: estado inicial
* q1: copiando los símbolos del primer término en la cinta 2.
* q2: eliminando símbolos de la cinta 2 para obtener el resultado.
...
| -- | - | - | -- | -- | - |
| q0 | - | - | - | q1, (1, R), (1, R)  |
| q1 | - | - | - | q1, (1, R), (1, R)  | q2, (0, R), (B, L)
| q2 | qA, (B, S), (B, S) | qA, (B, S), (1, S) | q2, (1, R), (B, L)
...
ORDEN:
* `δ(q0, (B,B))`: qa (B,S), (1,S)
* `δ(qa, (B,1))`: qa (a,R), (1,R)   *// mientras el contador de a este en 1, escribo a*
* `δ(qa, (B,B))`: qb(B,S), (B,L)    *// encuentra B en 1ra pista y pasa a qb para empezar a escribir bs*
* `δ(qb, (B,1))`: qb(b,R), (1,L)

* `δ(qb, (B,B))`: qa(#,R), (1,S)    *// al encontre (B, B), se escribe el delimitador (“#”) y se incrementa el contador para prepararse para la siguiente cadena*


nunca hago qA o qR porque esto no termina de ejecutar, simplemente continua escribiendo infinitamente
